        ...
        <div className="seat-grid" style={gridStyle}>
          {map.cells.map((c) => (
            <div
              key={`${c.row}-${c.col}`}
              className={`seat ${
                c.is_aisle ? "aisle" : c.blocked ? "blocked" : c.occupied ? "occ" : "free"
              }`}
              title={c.blocked ? `R${c.row}C${c.col} 遮挡禁坐` : `R${c.row}C${c.col}`}
              style={
                !c.is_aisle && !c.blocked && c.heat
                  ? { boxShadow: `inset 0 0 0 1px rgba(255,180,80,${Math.min(0.9, c.heat / 10)})` }
                  : undefined
              }
            >
              {c.is_aisle ? "" : c.blocked ? "✕" : c.col}
            </div>
          ))}
        </div>
      )}
